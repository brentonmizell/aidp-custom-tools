...
Data Ops Toolkit
================
One tool that operates on tabular data (a list of record objects):

  DataManipulationTool - select / rename / sort / dedupe / groupby (pandas),
                         plus filter (keep records matching a condition) and
                         compare (diff two record sets: added/removed/changed).
                         Driven by the `operation` parameter.

Formerly a three-tool kit (FilterTool + CompareTool + DataManipulationTool);
filter and compare are now operations on the single tool so an agent sees one
data-ops function instead of three.

Data is passed as a list of objects or a JSON string encoding one.
...
# --------------------------------------------------------------------------- #
# Filter operation - merged into DataManipulationTool as operation="filter".
# --------------------------------------------------------------------------- #
_FILTER_OPS = {
    "eq": lambda a, b: a == b,
    "ne": lambda a, b: a != b,
...
def _op_filter(conf, runtime_params, context_vars):
    """Keep only the records where a field satisfies a condition.
    (operation="filter" on DataManipulationTool.)"""
    ...
    debug(f"data-ops filter start op={op} field={field!r} source_uri={source_uri!r} dest_uri={dest_uri!r}")
    ...
    if op not in _FILTER_OPS:
        msg = f"Unknown filter operator '{op}'. Valid: {', '.join(_FILTER_OPS)}"
        return _err(msg, "InvalidOperator", error=msg)
    if op not in ("is_null", "not_null") and not field:
        msg = "field is required for this filter operator"
    ...
    if op in ("gt", "gte", "lt", "lte"):
        try:
            ...
            msg = f"filter operator '{op}' requires a numeric value (got {value!r})"
    ...
    if truncated:
        debug_warn(f"data-ops filter truncated kept={len(kept)} -> {max_results}")
    if skipped:
        debug(f"data-ops filter skipped {skipped} uncomparable rows")
...
# --------------------------------------------------------------------------- #
# Compare operation - merged into DataManipulationTool as operation="compare".
# --------------------------------------------------------------------------- #
...
def _op_compare(conf, runtime_params, context_vars):
    ...
    dest_uri = (runtime_params.get("dest_uri") or "").strip()
    debug(
        f"data-ops compare start key={key!r} source_uri={source_uri!r} "
    ...
    if (left_payload is None or right_payload is None) and source_uri:
        ...
        if perr:
            return _err(perr, "InputError", error=perr)
        lp, rp = _split_paired_payload(payload)
    ...
    if not key:
        msg = "key (the id field to match records on) is required for compare"
        return _err(msg, "MissingKey", error=msg)

    # Tolerance: explicit 0 is respected (strict equality). Default 1e-9
    # guards against float-representation noise without hiding real changes.
    ...
    try:
        ...
        debug(f"data-ops compare {summary} tolerance={tolerance}")
        data = {
            "operation": "compare",
            "summary": summary,
            "added": added,
        ...
        # csv/jsonl dest gets the 'changed' list as rows (most useful single-
        # table view). Json dest gets the full envelope.
        envelope = _maybe_write_dest(envelope, dest_uri, conf, context_vars,
                                     rows_key="changed")
        return envelope
    except Exception as e:
...
# --------------------------------------------------------------------------- #
# Data manipulation (pandas) + filter/compare dispatch
# --------------------------------------------------------------------------- #
@CustomToolBase.register
class DataManipulationTool(CustomToolBase):
    """Reshape tabular data: select/rename columns, sort, dedupe, and group-by
    aggregate (pandas), plus filter (keep matching records) and compare (diff
    two record sets). Driven by an 'operation' parameter.

    operation:
      select   - keep spec.columns
      rename    - spec.map old->new
      sort      - spec.by (+ spec.ascending)
      dedupe    - spec.subset (optional)
      groupby   - spec.by + spec.agg
      filter    - keep rows where `field` `operator` `value` (see _FILTER_OPS)
      compare   - diff `left`/`right` record sets on `key`
    """
    ...
    def _execute_tool(cls, conf, runtime_params, **context_vars):
        ...
        # Merged operations carry their own I/O + param shapes; dispatch early.
        if op == "filter":
            return _op_filter(conf, runtime_params, context_vars)
        if op == "compare":
            return _op_compare(conf, runtime_params, context_vars)
